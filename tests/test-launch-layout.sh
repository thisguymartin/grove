...
git_layout="$(GROVE_WORKTREE_BACKEND=git AI_EDITOR=codex bash ./launch-worktrees.sh --layout-only .)"
assert_contains "$git_layout" 'format_right "#[fg=$tab_dim,bg=$bar] ai:codex · git #[fg=$cyan,bg=$bar]{session} "' "right bar shows agent and backend"
